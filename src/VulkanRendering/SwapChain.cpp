...
SwapChain::SwapChain(Device& device, Window& window, VkExtent2D windowExtend)
	: device(device), window(window), windowExtent(windowExtend)
{
	createSwapChain();
	createImageViews();
	createRenderPass();
...
SwapChain::SwapChain(Device& device, Window& window, VkExtent2D windowExtend, VkSwapchainKHR swapChain)
	: device(device), window(window), windowExtent(windowExtend)
{
...
void SwapChain::createSwapChain(VkSwapchainKHR OldswapChian)
{
	...
	if (vkCreateSwapchainKHR(device.Getdevice(), &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
		VK_LOG_ERROR("failed to create swap chain!");
		exit(0);
	}
...
void SwapChain::createImageViews()
{
	...
	for (size_t i = 0; i < swapChainImages.size(); i++) {
		...
		if (vkCreateImageView(device.Getdevice(), &createInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS) {
			VK_LOG_ERROR("failed to create image views!");
			exit(0);
		}
	}
